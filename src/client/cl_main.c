...
=================
*/
void CL_FlushMemory( qboolean resetUI ) {
	...
	if( resetUI )
	{
		Cvar_Set( "fs_game", cls.initialFSGame );
...
void CL_Frame ( int msec ) {
	...
	if ( cls.state == CA_DISCONNECTED && !( Key_GetCatcher( ) & KEYCATCH_UI )
		...
		// reset the ui, fs_game, and menus if necessary
		CL_ResetUI();
	}
...
Compare the vms/menus loaded and the state of fs_game to what the user would expect
them to currently be. If they are different, take the appropriate action to fix that. 
==================
*/
void CL_ResetUI(void) 
{
	// If the server changed us away from our original fs_game, reset it
	if( cls.serverChangedFSGame )
	{
		Com_DPrintf("CL_ResetUI: Reset because server changed fs_game.\n");
		Cvar_Set( "fs_game", cls.initialFSGame );
		cls.serverChangedFSGame = qfalse;
	}
	else
	{
		// If the server didn't change fs_game, but the user did (via the mods menu), or the
		// loaded VMS otherwise changed (sv_pure server loaded alternate vms without changing fs_game)
		if( uivm->codeLength != cls.initialUIHash )
		{
			//Mods-menu case: fs_game changed, but the server didn't do it
			if( Q_stricmp( Cvar_VariableString("fs_game"), cls.initialFSGame ) )
			{
				strcpy( cls.initialFSGame, Cvar_VariableString( "fs_game" ) );
				cls.initialUIHash = uivm->codeLength; 
				Com_DPrintf("CL_ResetUI: Reset by mods menu: Updating initalfsgame = %s\n", cls.initialFSGame);
			}
			else //VM changed but fs_game did not: a pure server with fs_game=base forced the load of a different vm set
			{
				Com_DPrintf("CL_ResetUI: Reset because vm changed, but no changes to fs_game.\n");
			}
